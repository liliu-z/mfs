...
def wait_state(mfs: MFS, identity: DocumentId, state: str, *, retired: bool = False) -> None:
    with mfs._condition:
        assert mfs._condition.wait_for(
            lambda: (
                mfs._targets[identity]["state"] == state
                and (not retired or not any(i == identity for i, _ in mfs._executing))
            ),
            10,
        )
...
def test_checkpoint_cancel_reopen_retry_and_atomic_artifact(tmp_path: Path) -> None:
    ...
    mfs = MFS.open(state, processors=[adapter])
    ...
    try:
        ...
        with mfs.open_artifact(identity, "transcript") as artifact:
            ...
            assert artifact.snapshot_id == mfs.query(select="doc").items[0].value.snapshot_id
...
def test_cancel_gate_survives_new_source_and_reopen(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert mfs.query(select="doc").items[0].value.text == "changed"
    finally:
        mfs.close()
...
def test_wait_is_scoped_and_durable_and_receipts_use_global_ready(tmp_path: Path) -> None:
    state = tmp_path / "state"
    mfs = MFS.open(state, processors=[FailingText()])
    try:
        mfs.create_namespace("n", "internal")
...
def test_reuse_across_sync_calls_preserves_ids_and_reprocess_bypasses_process(
    ...
    try:
        ...
        mfs.wait(mfs.sync("n"), 10)
        assert processor.calls == embedder.calls == 1
        (root / "c.txt").write_text("reuse needle")
        mfs.wait(mfs.sync("n"), 10)
        assert {i.value.doc_id for i in mfs.query().items} == {"b.txt", "c.txt"}
        assert {
...
def test_gc_pins_open_artifacts_and_retains_cancelled_checkpoints(tmp_path: Path) -> None:
    ...
    mfs = MFS.open(tmp_path / "state", processors=[processor], gc_policy=policy)
    identity = DocumentId("n", "a.txt")
    try:
        mfs.create_namespace("n", "internal")
        ...
        mfs.cancel(identity)
        wait_state(mfs, identity, "cancelled", retired=True)
        checkpoint = mfs._targets[identity]["checkpoint"]["files"]["partial"]
        for _ in range(4):
...
def test_unsupported_sniff_is_bounded_and_case_rename_retires_old_spelling(
    ...
    def stage(descriptor: int):  # type: ignore[no-untyped-def]
...
def test_managed_process_retires_on_cancel_and_heavy_does_not_block_light(tmp_path: Path) -> None:
    adapter = NativeAdapter()
    adapter.workload = "heavy"
    adapter.media_types = ("application/test",)
    adapter.suffix_media_types = {".native": "application/test"}
    mfs = MFS.open(tmp_path / "state", processors=[adapter, Utf8TextProcessor()])
    try:
        mfs.create_namespace("n", "internal")
        native = mfs.upsert("n", "long.native", b"input")
        assert adapter.entered.wait(5)
        mfs.wait(mfs.upsert("n", "fast.txt", b"fast"), 10)
        with pytest.raises(WaitTimeout):
            mfs.wait(native, 0)
...
class P:
    ...
    def process(self, path, media, context):
        ...
        os._exit(29)
m=MFS.open(Path(sys.argv[1]),processors=[P()])
m.create_namespace("n","internal")
m.upsert("n","a.txt",b"source")
m.wait_ready(10)
assert m.query(select="doc").items[0].value.text=="recovered"
m.close()
"""
...
def test_delete_runs_with_mismatched_index_configuration(tmp_path: Path) -> None:
    from mfs import DefaultChunker
    ...
    mfs = MFS.open(state, processors=[Utf8TextProcessor()], chunker=chunker)
    try:
        assert mfs.status().index_state == "mismatch"
        assert not mfs.upsert("n", "a.txt", b"retire").index_ready
        mfs.wait(mfs.remove("n", "a.txt"), 10)
...
def test_corrupt_weak_cache_recomputes_and_path_dependent_adapters_do_not_share(
    ...
    try:
        ...
        assert processor.calls == 2
        assert all(i.value.text == "cache" for i in mfs.query(select="doc").items)
    finally:
        mfs.close()
    ...
    adapter = PathAdapter()
    mfs = MFS.open(tmp_path / "path-state", processors=[adapter])
    try:
        mfs.create_namespace("n", "internal")
        mfs.wait(mfs.upsert("n", "first.txt", b"same"), 10)
        mfs.wait(mfs.upsert("n", "second.txt", b"same"), 10)
        assert adapter.calls == 2
        assert {i.value.text for i in mfs.query(select="doc").items} == {"first.txt", "second.txt"}
    finally:
        mfs.close()
...
def test_gc_recovers_claimed_deletion_and_never_changes_task_state(tmp_path: Path) -> None:
    ...
    mfs = MFS.open(state, processors=[Utf8TextProcessor()], gc_policy=policy)
    try:
        mfs.create_namespace("n", "internal")
    ...
    try:
        ...
        assert mfs.query(select="doc").items[0].value.original == b"keep"
        mfs.wait(receipt, 0)
    finally:
        mfs.close()


def test_checkpoint_yields_capacity_to_active_scope(tmp_path: Path) -> None:
    from mfs import PreparationPolicy, UnderPath
    ...
    mfs = MFS.open(
        tmp_path / "state",
        processors=[PriorityAdapter()],
        preparation_policy=PreparationPolicy(light_workers=1),
    )
    try:
        mfs.create_namespace("n", "internal")
