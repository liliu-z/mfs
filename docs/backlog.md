...
以下编号对应用户确认的问题序号；[原审查](review-2026-09-13-boundary-audit.md)保留修复前证据，[修复与验收记录](review-2026-09-14-boundary-fixes.md)说明最终行为和测试。

- [x] **CLEANUP-008（第 1 项）** collection 删除成功且旧执行实际退出后，持久结清对应 namespace incarnation / collection generation 的清理责任，包括重试耗尽的记录；新建同名 namespace 的等待不得继承旧实例错误。验收删除完成、关闭重开、同名重建及迟到写入窗口。
- [x] **BOOTSTRAP-008（第 2 项）** 首次初始化中断可重开恢复，不将 MFS 自己留下的初始化文件直接判为损坏；只恢复可识别的自有初始化状态，继续拒绝无关的非空目录。覆盖 catalog 创建前和 schema 提交前的异常、进程终止。
- [x] **SYNC-WAIT-008（第 4 项）** sync 报告表达实际接收的 canonical 范围，wait(report) 等待该范围的当前目标，不能因别名拼写不同提前成功。覆盖文件符号链接，并验证 root 重定向导致实际扫描范围扩大时的报告。
- [x] **CONFIG-PARTIAL-008（第 6 项）** 支持健康成员发布新检索能力，单个失败成员不再阻止整个 namespace 首次启用；失败/取消成员保留诊断、取消意图和后续补齐能力，不混用不同 embedding space 的向量。明确首次启用与替换已有有效模型的切换语义，验收部分失败、取消、重试和重开恢复。
- [x] **BLOCKING-008（第 7 项）** 在现有状态接口区分缺少 Adapter 绑定、用户暂停、等待资源和等待旧执行退出，避免只报告无原因 pending；无法由后台自行解除的缺绑定等待提供明确诊断。补绑定/恢复/释放资源后状态自然收敛，不另建 ready 状态机。

- [x] **MAINTENANCE-008（第 3 项）** 两个有界维护线程按 namespace 调度，collection 调用锁分别持有，独立 collection 可并行，同 collection 保留固定后端要求的单 writer 保护。创建/加载、退休和独立清理受阶段期限监督；当前 Lite handler 不协作取消，保留真实调用至返回，超时拒绝迟到发布，不提前释放 drop/close 所依赖的占用。空转回归已从 0.2 秒内 853 次维护收敛为有间隔轮询。

随并发调整补齐：候选任务在领取前检查可复用的活动代文字，避免重建重复处理/checkpoint 恢复；绑定状态直接读取运行绑定表，不依赖 Worker 启动时序。

第 8 项（后端打不开时独立打开元数据/文字）暂缓。第 9 项（不能强制停止任意 Python/native Search 调用）保留现有边界，不新增修复。第 10 项的跨进程资源共享是可选接入方式，MFS 默认用自己的 LocalAdmission，不要求 Node 发许可。第 5、11–19 项中的 StashBase 排队、迁移、源操作、打包及其他宿主接入事项不纳入本轮 MFS 待修复；第 13 项来自假设的外部准备过渡方案，不是当前 MFS 缺陷。原有 BACKEND-005 仍按已有待办处理。

## 当前边界

- [x] **CLAIM-007** 领取失败共用三次预算和单调时钟退避；耗尽后停止调度并报告 StorageFailed，存储恢复后重开继续。提交确认丢失时采用已提交领取，不误计失败。
- [x] **STATUS-007** 状态页按文件使用清理索引检查 cleanup_pending，不重复读取和解码 namespace 全部清理记录。
- [x] **RULES-007** Internal 最终接收复查 SQLite 最新规则；重开将旧版本遗留的已排除输入转为删除任务。
- [x] **STATE-007** 内存刷新不写候选成员；配置维护从 SQLite 差异恢复新增、替换、删除和取消，任务更新事务拒绝缓存旧值覆盖持久新目标。
- [x] **CONFIG-007** 无有效准备文字的已取消输入保持取消并随配置晋升；后续由 CONFIG-PARTIAL-008 扩展为成功成员部分发布，继续保留所有用户取消意图。
...
2026-09-14，用户确认的边界修复：全量 **241 passed、3 skipped、1 xfailed**（620.24 秒），Ruff、格式、strict Pyright 通过。新增 23 项回归，覆盖首次初始化恢复、清理结账、维护并发/期限、部分配置发布和阻塞原因；实际 Milvus 并发与服务端慢 DDL、SIGKILL、跨重开重试均有验证。详见 [修复与验收记录](review-2026-09-14-boundary-fixes.md)。

2026-09-13，并发与恢复复审修复：新增 13 项回归，覆盖规则接收竞争、旧无效目标重开、领取写失败/退避/确认丢失、配置成员故障、旧候选遮蔽控制命令及无文字取消成员晋升。最终全量 **218 passed、3 skipped、1 xfailed**；ruff、格式、pyright 通过。Windows 原生测试和上游 BM25 已知限制保持原标记；详细证据见 [复审记录](review-2026-09-13-liveness.md)。
